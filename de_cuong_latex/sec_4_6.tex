...
tin cậy (đóng góp C3), hiện \textbf{chưa triển khai} --- đây là phần chính nhóm sẽ xây trong 24 tuần.}
\label{fig:pipeline}
\end{figure}
...
\begin{canhbao}[title={Nhóm tự nêu giới hạn của bảng này}]
Bảng \ref{fig:arch} chạy ở dải \textbf{3--90\,Hz}, không phải dải 10--60\,Hz tối ưu; đánh giá trên
\textbf{một} bản ghi validation với \textbf{một} seed; và F1 báo cáo là \textbf{giá trị lớn nhất} quét
trên 18 ngưỡng ngay trên chính bản ghi đánh giá. Vì vậy \textbf{các con số tuyệt đối đều lạc quan}.
Chúng chỉ dùng để so sánh \textbf{tương đối} giữa các kiến trúc, vì mọi kiến trúc đều chịu cùng một
thiên lệch.

Đọc đúng bảng này thì \texttt{cnn\_dil} xếp \textbf{hạng 5}, sau \texttt{resnet1d} và \texttt{tcn}.
Vì vậy nhóm \textbf{không} phát biểu ``TCN là kiến trúc tối ưu''. Lập luận bảo vệ được là
\textbf{hiệu quả tham số}: \texttt{cnn\_dil} đạt 99,6\% hiệu năng của quán quân với 28\% số tham số.
Chạy lại bảng này ở dải 10--60\,Hz theo giao thức tách bản ghi đầy đủ là công việc đầu tiên của Phase P1.
\end{canhbao}
