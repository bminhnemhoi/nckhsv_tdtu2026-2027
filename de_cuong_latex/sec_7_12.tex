...
Sụp đổ xuyên hệ ghi (CinC 2013: 77,34, lưỡng cực) &
\tot{Đã xử lý phần lớn} & Mô hình 22 ca đạt 90,34, không bản ghi nào dưới 50 (\S\ref{sec:model22}).
Khoảng cách còn lại 90 so với 97--99 là phần thật sự do thiết bị \\
Khoảng tin cậy 95\% của 99,21 với $n = 5$ vượt quá 100\% &
Trung bình & Với $n = 22$ vấn đề giảm; vẫn dùng bootstrap thay phân phối $t$ khi viết bài \\
...
(\S\ref{sec:phuongphap}). Còn thiếu: ba seed và ba kiến trúc nặng (transformer, hai biến thể 2D) \\
Chưa cài lại baseline nào &
\tot{Đã xử lý} & Ba baseline cổ điển đã cài lại và chấm cùng giao thức (\S\ref{sec:baselines}). Power-MF
không chạy lại được vì cần MATLAB và mã đa kênh chưa công bố \\
Đóng góp C3 chưa có dữ liệu & \tot{Đã xử lý} & Prototype đã chạy. Phần topo cho kết quả phủ định, phần cổng
từ chối có tác dụng (\S\ref{sec:fsqi}, \S\ref{sec:demo}) \\
...
\item[C3 --- đã chạy prototype; nửa khẳng định, nửa phủ định.]
Phần \textit{cổng từ chối} có tác dụng thật: $+7{,}4$ điểm F1 ở độ phủ 80\% trên CinC 2013 so với từ
chối ngẫu nhiên, và bản demo bắt đúng hai bản ghi tệ nhất mà không bỏ lọt bản ghi tốt nào. Phần
\textit{đồng điều bền vững} thất bại có kiểm soát: AUROC 0,566 so với 0,929 của chỉ số cổ điển
(\S\ref{sec:fsqi}). Nhóm rút lại phần topo và giữ phần cổng. Điểm ``đầu tiên'' còn lại là: thử nghiệm có
kiểm soát đầu tiên của TDA trên tín hiệu tim thai --- với kết quả phủ định. Yếu hơn về mặt tiếp thị,
nhưng đúng, và giả thuyết H0 đã dự phòng cho đúng tình huống này.
...
ba kiến trúc nặng.
\item \textbf{P2} xong: ba baseline cổ điển (\S\ref{sec:baselines}). Power-MF không chạy lại được, đã ghi lý do.
\item \textbf{P4--P6} prototype xong với kết quả phủ định cho topo và khẳng định cho cổng từ chối
(\S\ref{sec:fsqi}). Việc còn lại là thay luật cứng trong demo bằng bộ phân loại cổ điển.
\item \textbf{P8} phần lớn: đường cong F1--SNR với nhiễu MIT-BIH (\S\ref{sec:snr}) và đường cong hiệu quả
mẫu (\S\ref{sec:model22}) xong. Còn ba seed cho bảng kiến trúc.
...
kháng nội bộ (\texttt{survey/review\_round2.md}: không phát hiện số sai; đã sửa 8 lỗi trình bày).
\item \textbf{P3} xong: huấn luyện lại trên 22 sản phụ, 11 fold theo nhóm, có tăng cường dữ liệu, 190 phút
CPU; 12 checkpoint mới (\S\ref{sec:model22}). Còn: đo riêng đóng góp của tăng cường; hiệu chuẩn lại cổng
tin cậy cho mô hình 22 ca.
\end{itemize}
Chưa động tới: P7 (tiền huấn luyện FECGSYNDB, cần 40 GB --- sau kết quả 22 ca, có thể không còn cần
thiết), P11 (nộp bài).
\end{ghichu}
...
\item \textbf{Một cổng từ chối đã đo được} cho C3: $+7{,}4$ điểm F1 ở độ phủ 80\% ngoài miền, kèm một
kết quả phủ định có kiểm soát về TDA. Đây là bằng chứng, không phải phỏng đoán.
\end{enumerate}
