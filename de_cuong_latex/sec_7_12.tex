...
\tot{Đã xử lý một phần} & Bộ Silesia đã tích hợp: 22 sản phụ độc lập, trong đó 10 thai kỳ 32--42 tuần
(\S\ref{sec:silesia}). \textbf{Huấn luyện} vẫn chỉ trên 5 ca; huấn luyện lại trên 22 ca là Phase P3 \\
...
\item \textbf{P8} một phần: đường cong F1--SNR với nhiễu MIT-BIH xong (\S\ref{sec:snr}); đường cong hiệu
quả mẫu đang chạy.
...
\item \textbf{P3} đang chạy: huấn luyện lại trên 22 sản phụ, 11 fold theo nhóm, có tăng cường dữ liệu,
khoảng 3 giờ CPU; kết quả sẽ vào phiên bản kế tiếp.
\end{itemize}
Chưa động tới: P7 (tiền huấn luyện FECGSYNDB, cần 40 GB), P11 (nộp bài).
\end{ghichu}
