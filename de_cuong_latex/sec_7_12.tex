...
(\S\ref{sec:cinc75}). Đó mới là nơi đáng đầu tư. Và mục tiêu định lượng đã rõ: quy tắc chọn kênh một
mình đang mất \textbf{7,47 điểm} so với oracle ngoài miền, trong khi trong miền chỉ mất 0,02--1,90 điểm.
...
trên mẫu 10 bản ghi và \textbf{sai} trên toàn bộ 75: vẫn còn \textbf{16/75 bản ghi dưới 50} với mô hình
22 ca (\S\ref{sec:cinc75}). Đây là điểm yếu số một của đề tài \\
...
\subsection{Nhóm đã đoán sai nguyên nhân hai lần trong một ngày}
\label{sec:doansai}
...
Quy tắc kênh 0 rơi xuống \xau{69,33}, trong khi quy tắc PSD mù nhãn đạt \tot{79,40} \textit{(cả hai số này
tính trên 75 bản, đã rút ở bản 3.5; trên 60 bản sạch: kênh 0 = 61,78, PSD = 74,28 --- kết luận định tính
giữ nguyên)}. Kênh 0 hoá ra là
...
\xau{Cột ``mẫu 10 bản ghi'' (69,31 và 90,34) là các con số ĐÃ BỊ RÚT}, giữ lại chỉ để đối chiếu.
...
\midrule
PSD mù nhãn (\textbf{số chính}) & \xau{69,31} & \tot{79,40} & $-10{,}09$ \\
...
\textbf{đánh giá CinC 2013 trên toàn bộ 75 bản ghi} (\S\ref{sec:cinc75}) xong. Còn ba seed cho bảng
kiến trúc.
...
\end{itemize}
Chưa động tới: P7 (tiền huấn luyện FECGSYNDB, cần 40 GB --- sau khi thấy 16/75 bản ghi CinC vẫn dưới 50,
...
\item \textbf{Tạp chí} --- \textit{IEEE JBHI} hoặc \textit{Biomedical Signal Processing and Control}
(Q1 Scimago) nếu có được bộ dữ liệu thứ ba có nhãn thật để xác nhận quy tắc chọn kênh; nếu không,
\textit{Physiological Measurement} (sân nhà của fECG, nhưng là \textbf{Q2/Q3 Scimago 2024, không phải Q1}
như bản 3.4 ghi): bài đầy đủ gồm C1, C2, C3.
