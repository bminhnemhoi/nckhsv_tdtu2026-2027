...
*Sửa sự thật 17/09/2026 (không đổi chiến lược): khoảng cách "17,92" đã rút → 23,28 theo PSD; cổng từ chối không độc lập với mạng; 0,934 ghi rõ 11/22 chủ thể; số STV ngoài miền đã rút; mức ngẫu nhiên nhóm lỗi (a) lấy đúng hàng PSD.*
...
| 6 | **Thích nghi miền thất bại có chẩn đoán**: notch thích nghi +0,25 (p 0,68), tự huấn luyện −0,67, AdaBN −1,58, TENT −2,43; áp ba dịch chuyển đo được lên 22 chủ thể chỉ mất 0,01 điểm, trong khi khoảng cách trong/ngoài miền là 23,28 điểm theo PSD (97,56 − 74,28; 15,55 theo peakprob hậu kiểm) [F] — số cũ "17,92" đã rút vì tính từ mốc 79,40 trên 75 bản nhiễm (`Z_DA_RUT.khoang_cach_trong_ngoai_mien_17_92`) → khoảng cách ngoài miền không phải loại dịch chuyển mà thích nghi không giám sát sửa được; nguyên nhân còn lại **chưa xác định** (phép thử nhìn thấy có âm tính giả 18,0 % [12,1; 25,0], `analysis/xacnhan_results.json`) [V7] | Kết quả âm tính + chẩn đoán cơ chế | `analysis/THICHNGHI.md`; các JSON trong `adapt/` (chưa cam kết git tại thời điểm vòng 6) | JBHI, CBM, BSPC nhận kết quả âm tính **nếu** nó phục vụ lập luận chính (ở đây: có). Physiol. Meas. rất hợp. | (a) Bốn phương pháp, một hạt giống → không tách được "thật sự thất bại" khỏi "biến thiên hạt giống" cho các hiệu số < 1 điểm; TENT −2,43 và AdaBN −1,58 thì đủ lớn. (b) Không được viết "khoảng cách là do không có tín hiệu" như kết luận: phép thử nhìn thấy đã đo và có âm tính giả 18 % trên nhịp bắt đúng; phần dư địa "thiếu tín hiệu" không xác định được [V7]. (c) Phản biện ML sẽ hỏi "đã thử DANN / CORAL / test-time augmentation chưa?" — trả lời thẳng: chưa, và nêu lý do cơ chế. |
| 7 | **Phổ lỗi có mức ngẫu nhiên**: 6 nhóm; "bỏ nhịp dù có tín hiệu" 3,8 %/1,0 % (kênh PSD) so với ngẫu nhiên 14,7 %/10,3 % cùng hàng PSD (`analysis/chandoan_results.json` → `pho_loi.*_kenh_PSD.pct_null`; cặp 15,2 %/11,2 % cũ là hàng `peakprob`; cột CinC tính trên 75 bản nhiễm); jitter 3,76 ms [F]; kết luận cũ về nút thắt (mô hình hay tín hiệu) **đã rút**, câu hỏi để mở — phép thử nhìn thấy có âm tính giả 18,0 % trên 60 bản sạch, phần dư địa "thiếu tín hiệu" không xác định được (`analysis/XACNHAN.md`) | Khung phân tích lỗi (phương pháp phụ) | `analysis/chandoan_results.json` (`pho_loi`, `tran`), `analysis/chandoan_m1_jitter.json` | Mục "error analysis" ở JBHI/TBME sẽ được đánh giá cao vì hiếm ai làm mức ngẫu nhiên. | (a) Phép thử nhìn thấy có âm tính giả 18 % trên 60 bản sạch → *mọi* con số nhóm (b) phải in kèm khoảng bất định đó [V7]. (b) Trần năng lực n = 4 trong mẫu. (c) Nhóm "bản giới hạn cứng" của vòng cũ đã bị vòng 6 bỏ — không được lén đưa lại. (d) a54 là lỗi nhãn (37/144) — cần ghi rõ trong bảng, không dùng làm bằng chứng mô hình. |
| 8 | **Cổng từ chối**: LOSO 12 chỉ số (6 thuần tín hiệu, 4 tính trên nhịp mạng dò, 2 là xác suất mạng — **không độc lập với mạng**), xếp đúng 3 bản khó 1-2-3 (1/1540); AUROC trong bản ghi 0,934 [0,872; 0,981] trên 11/22 chủ thể có đoạn xấu, chỉ ở dạng phân tích, chưa đưa vào demo [F]; số zero-shot trên CinC trong `gate22_cinc.json` tính trên **75 bản gồm 15 bản nhiễm** — đã rút khỏi mọi văn bản, chưa tính lại trên 60 sạch [V7]; nhưng 5/24 quy tắc một đặc trưng cũng xếp đúng 3/3 [F] | Phương pháp (phụ) + đánh giá risk–coverage | `analysis/gate22_results.json` (`gate22_cinc.json` chỉ dùng sau khi tính lại trên 60 sạch) | JBHI (thích "trustworthy"/abstention), Physiol. Meas.; CinC. | (a) 12 chỉ số huấn luyện trên 22 chủ thể — với 3 bản khó, "1/1540" là sự kiện hiếm nhưng 5/24 đặc trưng đơn cũng đạt → cổng học được có thể chỉ tương đương một ngưỡng SNR. (b) Khi tính lại trên 60 sạch: ~9 bản không nhìn thấy tín hiệu ở kênh nào — phân biệt "không có gì để bắt" thì dễ; cái khó là bản trung gian. (c) Cổng không độc lập với mạng: độ quan trọng hoán vị `rr_cv` 0,140, `peak_prob_mean` 0,032, 10 chỉ số còn lại < 0,002 (`analysis/gate22_results.json` → `cong.permutation_importance_delta_auroc`) — phản biện sẽ hỏi cổng có sai cùng lúc với mạng không. *(Ghi chú `eff_bits` từng đặt ở đây thuộc bảng dịch chuyển miền của mục #6, không phải đặc trưng của cổng.)* |
...
| 10 | **Đo STV lâm sàng**: chệch +0,33 ms trong miền (ADFECGDB, n = 5); không dùng được làm máy đo STV độc lập [F]. Mọi số STV ngoài miền (+20,50 ms; 0,13 ms; 25,74–25,78 ms) đo trên mẫu có a03 a04 a05 a08 — bản trùng dữ liệu huấn luyện — **đã rút** | Kết quả âm tính (lâm sàng) | `analysis/clinical_results.json`, `analysis/CLINICAL.md` | Mục "clinical applicability" ngắn ở JBHI/Physiol. Meas.; là *hạn chế* chứ không là đóng góp. | (a) Chưa có số STV ngoài miền dùng được; phải đo lại trên 60 bản sạch — và bản CinC chỉ dài 1 phút, nên cũng chỉ đủ nói "dùng được / không", không đủ cho trị số Dawes-Redman. (b) Phản biện lâm sàng sẽ hỏi vì sao không đo trên nhịp *đã qua cổng* — nếu STV sau cổng tốt thì đây là lập luận cho cổng; nếu chưa làm, hãy làm (rẻ). |
...
6. **Why does the remaining gap persist?** — bốn DA thất bại; ba dịch chuyển đo được chỉ giải thích 0,01/23,28 điểm (khoảng cách theo PSD; 15,55 theo peakprob); phổ lỗi với mức ngẫu nhiên; phép thử nhìn thấy kèm âm tính giả 18,0 % [12,1; 25,0] — nguyên nhân khoảng cách còn lại để mở; jitter 3,76 ms [V7].
7. **Does the system know? Abstention** — 12 chỉ số (6 dựa trên đầu ra mạng; nêu thẳng cổng không độc lập với mạng), LOSO, risk–coverage (AUROC trong bản ghi 0,934 [0,872; 0,981] trên 11/22 chủ thể có đoạn xấu); số zero-shot CinC **chỉ in sau khi tính lại trên 60 bản sạch** (số hiện có đo trên 75 bản gồm 15 bản nhiễm — không trích) [V7]; so với quy tắc một đặc trưng (5/24 đạt 3/3 — tự khai); STV sau cổng (nếu làm) hoặc STV thất bại (nếu không).
...
| **8** (2–8/11) | (a) Viết mục 5, 6, 7. (b) Hình chính: (i) sơ đồ rò rỉ 5→15, (ii) F1 và logit theo quy tắc trên hai/ba bộ, (iii) risk–coverage, (iv) phổ lỗi có mức ngẫu nhiên. (c) Xin **góp ý lâm sàng** từ bác sĩ sản (qua giảng viên) về mục Discussion: cổng từ chối có chấp nhận được trong theo dõi không, độ phủ của cổng (tính lại trên 60 bản sạch) nghĩa là gì trong thực hành. | Bản nháp đầy đủ v1; 4 hình | Tuần 4–7 | Chủ nhiệm; **bác sĩ sản** (1 buổi) |
