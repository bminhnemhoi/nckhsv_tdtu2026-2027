...
| `test_core.py` | Kiểm thử pytest cho lõi (không cần gradio; 68 kiểm thử, toàn repo 111 cùng 43 trong `tests/`, đếm bằng `pytest --collect-only` 18/09/2026; số tăng theo các vòng sửa demo), gồm hai mốc bắt buộc chạy ở **cả hai** chế độ đèn (`luat` và `hoc`): r01 kênh 4 F1 > 99 & đèn *cao*; a02 đèn *không cao*. |
...
| `screenshot_v2.py` | Chế độ trình bày: chạy `python demo/app.py` rồi mở `/gradio/`, bấm 4 thẻ, đi hết 5 bước, ghi thời gian và chụp 8 ảnh `17..24` → `screenshots/screenshots_v2.json`. |
